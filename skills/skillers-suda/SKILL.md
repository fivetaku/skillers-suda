...
워크플로우 설계 결과를 AskUserQuestion의 markdown 프리뷰로 보여준다.

**EXECUTE:** 아래 JSON의 markdown 필드를 워크플로우 설계 결과(단계별 흐름 + 엣지 케이스 대응)로 채운 후 AskUserQuestion 도구를 즉시 호출한다:

```json
{
  "questions": [
    {
      "question": "이 워크플로우 어떻게 할까요?",
      "header": "워크플로우",
      "options": [
        {
          "label": "이대로 진행 (추천)",
          "description": "이 워크플로우대로 파일을 만들어드릴게요.",
          "markdown": "(동적: 단계별 흐름 + 엣지 케이스 대응 목록)"
        },
        {"label": "수정할 부분 있어", "description": "어떤 단계를 바꾸고 싶은지 알려주세요."},
...
**목표:** 만든 스킬을 테스트하고 개선합니다.

**테스트 시나리오 자동 생성:**

검수자 에이전트의 분석 결과(엣지 케이스, 테스트 시나리오)를 활용하여:
- 정상 경로 테스트 — 샘플 입력으로 전체 흐름 확인
- 엣지 케이스 테스트 — 검수자가 식별한 예외 상황
- 각 단계의 예상 출력
